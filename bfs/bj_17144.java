...
public class bj_17144 {
	...
	static int total; // 방에 있는 전체 미세먼지 양
	...
	static void bfs() {
		...
		while (!queue.isEmpty()) {
			...
			if (T == 0) {	// 주어진 시간이 모두 지나면 종료
				return;
			}
			
			// 확산된 미세먼지를 표시하기 위해 0으로 값 초기화(공기청정기는 제외)
			for (int i = 0; i < R; i++) {
				for (int j = 0; j < C; j++) {
					if(dust[i][j]>0) {
						dust[i][j]=0;
	...
	static void airClean() {
		...
		while (d < 4) {
			...
			if (nr>=0 && nr<=top && nc>=0 && nc<C) {
				if(dust[r][c]==-1) {	// 미세먼지가 공기청정기로 들어감
					dust[r][c]=-1;
				}else if(dust[nr][nc] == -1){	// 공기청정기에서 바람이 나옴
					dust[r][c]=0;
				}else {
					dust[r][c] = dust[nr][nc];
				}
